...
驾驶主屏采用 3:7 布局：左侧是驾驶员近侧 AURI 面板，右侧是持续可见的导航主画布。导航画布使用车道级放大图、立体街区、道路分级、POI、路况分段和路线进度表达 Demo 所需导航语义；它是可控的 HMI 模拟层，不是高德地图 SDK。主路线只保留一条连续路径，灰色表示已行驶、蓝色表示剩余路线，车辆位置和方向随 Demo 阶段平滑更新。主屏只保留一句判断、一个动作组、一个主要确认入口、ETA 和刚性/弹性任务摘要；速度、限速和档位放在地图右上角。消息草稿、方案、行程、车况和三端同步通过驾驶员侧原位二级页查看，不遮罩或压暗地图。

底部任务摘要使用“责任图标 + 责任类型 + 任务 + 状态标签”，刚性责任和弹性任务不会只靠颜色区分。行程摘要使用“大数字 + 小单位”展示剩余距离、预计用时和到达时间，1280×720 下不得截断关键值。
