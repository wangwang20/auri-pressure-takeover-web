...
驾驶主屏采用 3:7 布局：左侧是驾驶员近侧 AURI 面板，右侧是持续可见的导航主画布。页面启动时默认从 Agent `/v1/map-config` 获取高德 Web JS API 配置，加载高德真实 2D 底图、道路和 POI、驾车路线、实时交通图层及车辆沿线动画；Key 缺失、网络失败或额度保护触发时才回退 SVG 离线地图。两种地图共用同一套 Agent 状态覆盖层，灰色表示已行驶、蓝色表示剩余路线，黄色表示拥堵风险。主屏只保留一句判断、一个动作组、一个主要确认入口、ETA 和刚性/弹性任务摘要；速度、限速和档位放在地图右上角。消息草稿、方案、行程、车况和三端同步通过驾驶员侧原位二级页查看，不遮罩或压暗地图。
...
- `消息`：按老师/家人联系人分段，显示草稿状态、分段正文和“生成草稿 → 等待确认/已发送”进度。
